...
def sitk_read_raw(img_path, resize_scale=1): # 读取3D图像并resale（因为一般医学图像并不是标准的[1,1,1]scale）
    ...
    nda=ndimage.zoom(nda,[resize_scale,resize_scale],order=0) #rescale

    return nda

def tif_read_raw(img_path, resize_scale=1): # 读取3D图像并resale（因为一般医学图像并不是标准的[1,1,1]scale）
    nda = cv2.imread(img_path)
    # if nda is None:
    #     raise TypeError("input img is None!!!")
    # nda = sitk.GetArrayFromImage(nda)  # channel first
    # nda=ndimage.zoom(nda,[resize_scale,resize_scale],order=0) #rescale
